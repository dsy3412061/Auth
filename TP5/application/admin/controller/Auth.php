...
//            }
        ...
        foreach ($authList as $auth) {
            $query = preg_replace('/^.+\?/U', '', $auth);
              // dump($query);dump($mode);dump($auth);dump($name);
    ...
    protected function getAuthList($uid, $type) {
        ...
        //读取用户组所有权限规则
        $rules = Db::name($this->config['auth_rule'])->where($map)->field('condition,title')->select();
        ...
        foreach ($rules as $rule) {
            if (!empty($rule['condition'])) { //根据condition进行验证
                ...
                $condition && $authList[] = strtolower($rule['title']);
            } else {
                $authList[] = strtolower($rule['title']); //只要存在就记录
            }
        }
